...
const cb = require('../common/colorbrewer.js');
...
function update(chart, data, scales, xdimension) {
  ...
  var c = d3.scaleLinear().domain(d3.extent(innerArrayValues(data, chart.grid_colour_values))).range([0,1]);
  ...
  rect.enter().append("rect")
    ...
    .attr("fill", function(d) { if(chart.grid_colour && chart.grid_colour_values)
                                {
                                  if(chart.grid_colour === "heatmap")
                                  {
                                      return heatmapColour(c(d[chart.grid_colour_values]));
                                  }
                                  if(chart.grid_colour === "lesk" || chart.grid_colour === "phyre" || chart.grid_colour === "clustal" || chart.grid_colour === "taylor")
                                  {
                                      return cb[chart.grid_colour][d[chart.grid_colour_values]];
                                  }
                                  return("blue");
                                }
                                else
    ...
    .append("svg:title")
          .text(function(d, i) {  if(chart.grid_colour && chart.grid_colour_values)
                                  {
                                    if(chart.grid_colour === "heatmap")
                                    {
                                    ...
                                    if(chart.grid_colour === "lesk" || chart.grid_colour === "phyre" || chart.grid_colour === "clustal" || chart.grid_colour === "taylor")
                                    {
                                      return d[chart.grid_colour_values];
                                    }
                                  }});
