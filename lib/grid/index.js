...
//
export function biod3_psipredGrid(data, opts={})
{
  let psipred_opts = {annotation_sets: annotations, download_buttons: true, grid_label: true, grid_colour_type: "custom", grid_colour_annotations: ["ss", "disopred", "dompred"], custom_grid_palette: custom };
  psipred_opts = Object.assign(psipred_opts, opts);
  biod3_genericGrid(data, 'res', 50, "psipredChart", psipred_opts);
}

export function biod3_heatmap(data, cellseries, xdimension, colour_values, opts={})
{
  let heatmap_opts = {grid_label: false, grid_colour_type: "heatmap", grid_colour_annotations: [colour_values] };
...
export function biod3_genericGrid(data, cellseries, xdimension, label="seqAnnotationChart", opts={})
{
  ...
  const scales = setScale(chart, [1,xdimension], [1, ydimension], ydimension*(chart.em_size+4), xdimension, ydimension, cell_number);
  if(opts.debug === true)
  {
    printDebug(chart);
  }
  drawAxis(chart, 'axes', scales.x, scales.y, scales.x_top, scales.y_right, ydimension*(chart.em_size+4));
  ...
  if(chart.download_buttons)
  {
    ...
    button_helper(chart, "svg", chart.em_size*5);
...
function drawLegend(chart)
{
  ...
  if(chart.grid_colour_type === 'phyre')
  {
    palette = cb.phyre_annotations;
  }
  ...
  if(chart.custom_grid_palette && chart.grid_colour_type === 'custom')
  {
    ...
    key.enter().append("rect")
      ...
      .attr("transform", function(d, i){return "translate("+[0, i*chart.em_size*1.4]+")";})
    ...
    let text = chart.key.selectAll(".text")
        .data(Object.keys(palette));
    text.enter().append("text")
      .attr("transform", function(d, i){return "translate("+[chart.em_size*1.5, (i*chart.em_size*1.4)+chart.em_size/1.5]+")";})
...
function update(chart, data, scales, xdimension) {
  ...
  let rollOverText = function(d, i) { let annotation_str = "Pos: "+(i+1)+"\n";
                          if(chart.grid_colour_type && chart.grid_colour_annotations)
                          {
                            ...
                            if(chart.grid_colour_type === "lesk" || chart.grid_colour_type === "phyre" ||
