...
function add_annotation_toggle(chart, data, scales, xdimension)
{
   ...
   .on('mouseover', function(d){
      d3.select('#'+d.name+'button')
        .attr('fill', '')
        .classed('active', true);
      d3.select('#'+d.name+'Text')
        .attr('fill', '')
        .classed('active', true);})
  ...
  let text = chart.annotationToggle.selectAll(".text")
               .data(chart.annotation_sets);
  const Txtbutton = function(d){return(d.name+"Text");};
  text.enter().append("text")
    ...
    .attr("dy", ".35em")
    .attr("transform", function(d, i){return "translate("+[i*chart.em_size*10, 0]+")";})
    .text(function(d){if(d.label){return "Show "+d.label;}else{return "Show "+d.name;}})
    .on('mouseover', function(d){
        d3.select('#'+d.name+'button')
          .attr('fill', '')
          .classed('active', true);
