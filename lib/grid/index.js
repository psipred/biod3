...
function add_annotation_toggle(chart, data, scales, xdimension)
{
    ...
    const button = function(d){return(d+"button");};
   ...
   .on('mouseover', function(d){
    ...
    .on('mouseout', function(d){
      ...
      d3.select('#'+d+'Text')
        .classed('active', false);})
    .on('click', function(d){console.log("Yo");chart.grid_colour_type=d;
                              chart.grid_colour_annotations=["res"];
  ...
  let text = chart.annotationToggle.selectAll(".text")
               .data(chart.annotation_sets);
  const Txtbutton = function(d){return(d+"Text");};
...
function drawLegend(chart)
{
  if(chart.custom_grid_palette)
  {
    let key = chart.key.selectAll(".rect")
      .data(Object.keys(chart.custom_grid_palette));
    key.enter().append("rect")
      .attr("class", "rect")
      .attr("width", 15)
      .attr("height", 15)
      .attr("transform", function(d, i){return "translate("+[0, i*20]+")";})
      .attr("stroke", function(d){const colour="white"; if(chart.custom_grid_palette[d].stroke){return chart.custom_grid_palette[d].stroke;} return colour;})
      .attr("fill", function(d){const colour="white"; if(chart.custom_grid_palette[d].fill){return chart.custom_grid_palette[d].fill;} return colour;});

    let text = chart.key.selectAll(".text")
        .data(Object.keys(chart.custom_grid_palette));
    text.enter().append("text")
      .attr("transform", function(d, i){return "translate("+[20, (i*20)+12]+")";})
      .text( function (d) { return chart.custom_grid_palette[d].name; })
