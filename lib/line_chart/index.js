...
export function biod3_genericxyLineChart(data, xseries, yseries, lineColours, label='xyLineChart', opts={})
{
  ...
  {
    draw_assignment_ribbon(data, chart, scales);
  }
  if(chart.x_cutoff)
  {
...
function draw_assignment_ribbon(data, chart, scales)
{
  ...
  //
  const stuff = data.reduce(function (arr, val) { return arr.concat( {x: val.pos,  y: 1} ); }, []);
  console.log(stuff);
}
